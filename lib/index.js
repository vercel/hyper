...
import {getFileDataAsync} from './utils/file';
...
import {loadConfig, reloadConfig, loadAsyncConfig} from './actions/config';
...
const fetchFileData = configData => {
  if (configData.bell !== 'SOUND' || !configData.bellSoundURL) {
    store_.dispatch(loadAsyncConfig({bellSound: null}));
    return;
  }

  getFileDataAsync(configData.bellSoundURL).then(bellSound => {
    const newConf = {bellSound};
    store_.dispatch(loadAsyncConfig(newConf));
  });
};
