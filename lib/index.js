...
rpc.on('tab created', ({uid, position}) => {
  store_.dispatch(sessionActions.newTab(uid, position));
});

rpc.on('tab change', ({uid}) => {
  store_.dispatch(sessionActions.tabChange(uid));
});

rpc.on('tab exit', ({uid}) => {
  store_.dispatch(sessionActions.tabExit(uid));
});

rpc.on('tab position', ({uid, position}) => {
  store_.dispatch(sessionActions.tabPosition(uid, position));
});
