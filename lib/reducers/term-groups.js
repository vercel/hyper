...
import {TERM_GROUP_EXIT, TERM_GROUP_RESIZE, TERM_GROUP_TOGGLE_MAXIMIZED_ACTIVE} from '../constants/term-groups';
...
const toggleGroupMaximized = (state, uid) => {
  // Make sure we have more than one pane in tab:
  if (state.termGroups[state.activeRootGroup].children.length < 2) {
    return state;
  }

  const maximized = state.termGroups[uid].maximized;
  return state.setIn(
    ['termGroups', uid, 'maximized'],
    !maximized
  );
};
...
const reducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case TERM_GROUP_TOGGLE_MAXIMIZED_ACTIVE:
      return toggleGroupMaximized(state, action.uid);
