...
export function left(state, pane) {
...
export function right(state, pane) {
  const display = state.displays[state.active.display];
  const indexOf = display.panes.indexOf(pane);
  console.log(display);
  console.log(indexOf, display.panes.length);
  if(indexOf+1 <= display.panes.length - 1 ) {
    return setActive(state, display.panes[indexOf+1]);
  }
}
export function down(state, pane) {
  return state;
}
export function up(state, pane) {
  return state;
}
// 
