...
import {create, activeDisplay, splited, paneIndex, setActive,
  left, right, down, up, setPrev, setNext} from './display';
...
export function swith(state, action) {
  ...
  const panes = Object.keys(state.panes).map(uid => state.panes[uid]);
  const index = state.panes[state.active.pane].num;
  switch (action.arrow) {
    case 'Left':
    ...
    case 'Prev':
      return setPrev(state, panes, index);
    case 'Next':
      return setNext(state, panes, index);
    default:
      return state;
  }
}
