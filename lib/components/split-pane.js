...
export default class SplitPane extends Component {
  ...
  template(css) {
    ...
    const {direction, borderColor, backgroundColor} = this.props;
    let {sizes} = this.props;
    if (!sizes) {
    ...
    return (<div className={css('panes', `panes_${direction}`)}>
      {
        React.Children.map(children, (child, i) => {
          const {maximized} = child.props.termGroup;
          const style = {
          ...
          return [
            <div
              key="pane"
              className={css('pane')}
              style={maximized ? maximizedStyle : style}
              >
              { child }
              { maximized &&
                <div className={css('maximized_icon')}>
                  <div
                    className={css('maximized_icon_inner_dot')}
