...
export default class Header extends PureComponent {
  ...
  template() {
    ...
    return (
      <header
        className={`header_header ${isMac && 'headerRounded'}`}
        onMouseDown={this.handleHeaderMouseDown}
        onDoubleClick={this.handleMaximizeClick}
      >
        {!isMac && (
          <div className={`header_windowHeader ${props.tabs.length > 1 && 'windowHeaderWithBorder'}`} style={{borderColor}}>
            {hambMenu && (
              <svg
                className={`shape ${(left && 'hamburgerMenuRight') || 'hamburgerMenuLeft'}`}
            ...
            {winCtrls && (
              <div className={`windowControls ${left && 'windowControlsLeft'}`}>
                <svg className={`shape ${left && 'minimizeWindowLeft'}`} onClick={this.handleMinimizeClick}>
                  <use xlinkHref="./renderer/assets/icons.svg#minimize-window" />
                </svg>
                <svg className={`shape ${left && 'maximizeWindowLeft'}`} onClick={this.handleMaximizeClick}>
                  <use xlinkHref={maxButtonHref} />
                </svg>
                <svg className={`shape closeWindow ${left && 'closeWindowLeft'}`} onClick={this.handleCloseClick}>
