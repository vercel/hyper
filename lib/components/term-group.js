...
class TermGroup_ extends Component {
  ...
  renderTerm(uid) {
    ...
    // debounced call let sufficient time for hterm to redraw without glitch
    const debouncedOnResize = debounce(this.props.onResize, 50);
    const props = getTermProps(uid, this.props, {
      ...
      onResize: this.bind(debouncedOnResize, null, uid),
