...
import {findRootGroup} from '../utils/term-groups';
...
class TermGroup_ extends Component {
  ...
  renderTerm(uid) {
    ...
    const index = this.props.sortedSessionGroups.indexOf(this.props.termGroup.uid);
    // Only 1-9 keys are used and if there is more than 9 terms, number 9 is reserved to the last term
    const termShorcutNum = index < 8 ? index + 1 : (index === this.props.sortedSessionGroups.length - 1 ? 9 : 0);
