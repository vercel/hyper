...
import { isValidUrl } from '../utils/url-regex';
import { arrBuf2Base64 } from '../utils/array';
...
const getFileData = (soundFileLocation) => {
  return new Promise( async (resolve, reject) => {
    const isRemoteUrl = isValidUrl(soundFileLocation);
    if (isRemoteUrl) {
      let response;
      let fileBlob;
      try {
        response = await fetch(soundFileLocation);
        if (!response.ok) {
          console.warn('There was an error attempting to retrieve the file from the url:', response.statusText);
          return resolve(null);
        }

        fileBlob = await response.blob();
      } catch (e) {
        console.warn('There was a problem attempting to retrieve the file from the web:', e);
        return resolve(null);
      }
      ...
      reader.addEventListener('loadend', () => {
        ...
        const base64Data = arrBuf2Base64(buffer);
        return resolve(base64Data);
      });

      reader.addEventListener('error', () => {
        console.warn('There was an error while attempting to read the file from the web:', reader.error);
        return resolve(null);
      })

      reader.readAsArrayBuffer(fileBlob);
    } else {
      // Might be a local file we are trying to use
      return fs.readFile(soundFileLocation, (err, data) => {
        if (err) {
        ...
        const base64Data = arrBuf2Base64(data);
...
export default class Term extends React.PureComponent {
  ...
  async componentWillReceiveProps(nextProps) {
    ...
    if (nextProps.bellSoundURL) {
      const value = await getFileData(nextProps.bellSoundURL);
      if (value) {
        nextTermOptions.bellSound = value;
      }
    }
