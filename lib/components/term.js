...
export default class Term extends PureComponent {
  ...
  getTermOptions(props) {
    ...
    return {
      ...
      theme: {
        ...
        brightBlack: props.colors.lighttBlack,
        brightRed: props.colors.lighttRed,
        brightGreen: props.colors.lighttGreen,
        brightYellow: props.colors.lighttYellow,
        brightBlue: props.colors.lighttBlue,
        brightMagenta: props.colors.lighttMagenta,
        brightCyan: props.colors.lighttCyan,
        brightWhite: props.colors.lighttWhite
      }
    };
  }
  ...
  componentDidMount() {
    ...
    const termOptions = this.getTermOptions(props);
    this.term = props.term || new Terminal(termOptions);
    this.term.attachCustomKeyEventHandler(this.keyboardHandler);
    this.term.open(this.termRef);

    this.onOpen(termOptions);
  ...
  componentWillReceiveProps(nextProps) {
    ...
    const termOptions = this.getTermOptions(nextProps);

    // Update only options that have changed.
    Object.keys(termOptions)
      .filter(option => option !== 'theme' && termOptions[option] !== this.term.getOption(option))
      .forEach(option => this.term.setOption(option, termOptions[option]));

    // Do we need to update theme?
    const currentTheme = this.term.getOption('theme');
    const shouldUpdateTheme =
      !currentTheme ||
      Object.keys(termOptions.theme).some(option => {
        termOptions.theme[option] !== currentTheme[option];
      });
    if (shouldUpdateTheme) {
      this.term.setOption('theme', termOptions.theme);
    }
