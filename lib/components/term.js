...
import {BELL_STYLE} from '../constants/config';
...
const getTermOptions = props => {
  ...
  return {
    ...
    bellStyle: props.bell === BELL_STYLE.SOUND ? 'sound' : 'none',
    theme: {
...
export default class Term extends React.PureComponent {
  ...
  componentDidMount() {
    ...
    this.termDefaultBellSound = this.term.bellSound;
  ...
  componentWillReceiveProps(nextProps) {
    ...
    // Check nextProps for the bellSoundURL field explicitly to see if there is a value
    // Only if there is a value do we want the field "bellSound" to exist in extTermOptions
    // If "bellSound" is set to null or undefined, the below logic will call "this.term.setOptions" with
    // the endesired value of null or undefined which causes xterm.js to fail tryign to play the bell sound.
