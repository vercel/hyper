...
import rpc from '../rpc';
import {getRegisteredKeys} from '../command-registry';
...
class Hyper extends PureComponent {
  ...
  attachKeyListeners() {
    ...
    Object.keys(keys).forEach(commandKeys => {
      console.log(`Register ${commandKeys} =>  ${keys[commandKeys]}`);
      this.mousetrap.bind(
        commandKeys,
        e => {
          console.log('Emit', keys[commandKeys], e);
          rpc.emit('command', keys[commandKeys]);
          e.catched = true;
        },
        'keydown'
      );
