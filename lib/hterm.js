...
hterm.Terminal.prototype.find = function (searchTerm, options) {
  return search(this, searchTerm, options);
};

hterm.Terminal.prototype.selectRow = function (selectionInfo, options) {
  return selection.selectRow(this, selectionInfo, options);
};

// scroll either up or down to a row, if it's currently off screen
hterm.ScrollPort.prototype.scrollTo = function (row, padding = 0) {
  const topRowIndex = this.getTopRowIndex();
  const bottomRowIndex = this.getBottomRowIndex(topRowIndex);

  if (row + padding > bottomRowIndex) {
    this.scrollRowToBottom(row + padding);
  } else if (row - padding < topRowIndex) {
    this.scrollRowToTop(row - padding);
  }
};
