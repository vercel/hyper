...
const loadModules = () => {
  ...
  modules = paths.plugins.concat(paths.localPlugins)
    .map(path => {
      ...
      if (mod.mapSearchboxState) {
        connectors.Searchbox.state.push(mod.mapSearchboxState);
      }

      if (mod.mapSearchboxDispatch) {
        connectors.Searchbox.dispatch.push(mod.mapSearchboxDispatch);
      }
