...
const loadModules = () => {
  ...
  reducersDecorators = {
    ...
    reduceTabs: tabsReducers,
    reducePanes: panesReducers,
  ...
  modules = paths.plugins.concat(paths.localPlugins)
    .map(path => {
      ...
      if (mod.reduceTabs) {
        tabsReducers.push(mod.reduceTabs);
      }

      if (mod.reducePanes) {
        panesReducers.push(mod.reducePanes);
      }
...
export function decorateTabsReducer(fn) {
  return decorateReducer('reduceTabs', fn);
}

export function decoratePanesReducer(fn) {
  return decorateReducer('reducePanes', fn);
}
