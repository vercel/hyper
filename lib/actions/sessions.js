...
export function setBroadcast(broadcast) {
  return {
...
const findRootChildren = (termGroups, session, children = []) => {
  if (session.parentUid === null) {
    return children
      .concat(session.children)
      .map(child => termGroups[child].sessionUid);
  }

  if (session.children.length > 0) {
    children = children.concat(session.children);
  }

  const parentSession = termGroups[session.parentUid];

  return findRootChildren(termGroups, parentSession, children);
}
  ...
  return function (dispatch, getState) {
    dispatch({
      ...
      effect() {
        ...
        const targetUid = uid || sessions.activeUid;
        const currentSession = findBySession(termGroups, targetUid)
        const group = sessions.broadcast === 'tab' ? findRootChildren(termGroups.termGroups, currentSession) : null;
        const broadcast = sessions.broadcast;
        rpc.emit('data', {uid: targetUid, data, broadcast, group});
      }
    });
  };
}
