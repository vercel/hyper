  ...
  return dispatch => {
    dispatch({
      ...
      effect() {
        ...
        let regExec = userRegExp.exec(url);
        if (regExec != null) user = regExec[1];

        regExec = portRegExp.exec(url);
        if (regExec != null) port = regExec[1];

        regExec = pathRegExp.exec(url);
        if (regExec != null) path = regExec[1];
