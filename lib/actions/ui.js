...
export function showPreferences() {
  ...
  if (isWin && cfg.shell.includes('bash')) {
    command = ' nano .hyper.js';
    message.push('echo Attempting to open .hyper.js with nano');
  } else if (isWin) {
    command = ' start notepad "%userprofile%\\.hyper.js"';
    message.push('echo Attempting to open .hyper.js with notepad');
  } else { // eslint-disable-next-line no-template-curly-in-string
    command = ' bash -c \'exec env ${EDITOR:=nano} ~/.hyper.js\'';
    message.push('echo Attempting to open ~/.hyper.js with your \\$EDITOR');
  }
