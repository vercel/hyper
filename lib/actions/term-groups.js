...
const findFirstSession = (state, group) => {
  ...
  for (let childUid of group.children) {
    ...
    dispatch({
      ...
      effect: () => {
        if (Object.keys(termGroups.termGroups).length <= 1) {
          return dispatch(setActiveSession(null));
        }

        const group = termGroups.termGroups[uid];
        const { activeSessionUid } = termGroups.termGroups[termGroups.activeRootGroup];
