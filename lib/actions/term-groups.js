...
export function restorePane(uid, pane) {
  return (dispatch, getState) => {
    const {ui} = getState();
    dispatch({
  ...
  return (dispatch, getState) => {
    ...
    dispatch({
      ...
      console.log(termGroups);
        ...
        if(group.children) {
          console.log('HAS children: ', group.children);
        }
        if (Object.keys(termGroups.termGroups).length <= 1) {
          console.log('termGroups.termGroups).length <= 1');
        ...
        if (group.sessionUid) {
          console.log('group.session CALLED: ', group.sessionUid);
          dispatch(userExitSession(group.sessionUid));
        } else {
          console.log(group);
          if(group.children) {
            console.log('ELSE GROUP: ',group);
            console.log('ELSE GROUP-session: ', group.sessionUid);
            console.log('ELSE GROUP-child: ', group.children);
