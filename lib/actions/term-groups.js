...
import confirmClose from '../utils/dialogs';
...
export function userExitTermGroup(uid) {
  ...
  return (dispatch, getState) => {
    const {termGroups} = getState();
    let activeSessionUid = termGroups.activeSessions[termGroups.activeRootGroup];
    let pid = getState().sessions.sessions[activeSessionUid].pid;

    let userCanceled = confirmClose(pid);
    if (userCanceled)
      return;
