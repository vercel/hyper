...
import {
  DIRECTION,
  TERM_GROUP_RESIZE,
  TERM_GROUP_TOGGLE_MAXIMIZED,
  TERM_GROUP_TOGGLE_MAXIMIZED_ACTIVE,
  TERM_GROUP_REQUEST,
  TERM_GROUP_EXIT,
...
export function requestPaneMaximizedStateToggle() {
  return (dispatch, getState) => {
    dispatch({
      type: TERM_GROUP_TOGGLE_MAXIMIZED,
      effect() {
        const {sessions, termGroups} = getState();
        const {uid} = findBySession(termGroups, sessions.activeUid);
        dispatch({
          type: TERM_GROUP_TOGGLE_MAXIMIZED_ACTIVE,
          uid
        });
      }
    });
